...
var Loger = function(options) {
    ...
    var now = new Date();
    var dateTime = '.' + now.getFullYear() + (now.getMonth() + 1) + now.getDate() + now.getHours();
    this.curLogs = {};

    for (var i in logs) {
        var access_log = logs[i].access_log + dateTime;
        this.logsStream[i] = fs.createWriteStream(access_log, {
            flags: 'a',
            encoding: 'utf8'
        });
        this.curLogs[i] = dateTime;
        if (logs[i].handler) {
...
Loger.prototype.access = function(req) {
    ...
    if (stream = this.logsStream[basename]) {
        ...
        if (typeof this.logsHandler[basename] === 'function') {
            data = this.logsHandler[basename](data, this.options.logs[basename]);
        }
        if (!data) {
            return;
        }
        ...
        var now = new Date();
        var dateTime = '.' + now.getFullYear() + (now.getMonth() + 1) + now.getDate() + now.getHours();
        if (this.curLogs[basename] === dateTime) {
            stream.write(result);

        } else {
            stream.end(result);
            var access_log = this.options.logs[basename].access_log + dateTime;
            this.logsStream[basename] = fs.createWriteStream(access_log, {
                flags: 'a',
                encoding: 'utf8'
            });
            this.curLogs[basename] = dateTime;
        }
